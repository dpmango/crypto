...
(function() {
    ...
    function B(a, c, b, d, f) {
        ...
        "axisX" === b ? ("left" === d || "bottom" === d ? (this.optionsName = "axisX", x(this.chart.sessionVariables.axisX[f]) && (this.chart.sessionVariables.axisX[f] = {}), this.sessionVariables = this.chart.sessionVariables.axisX[f]) : (this.optionsName = "axisX2", x(this.chart.sessionVariables.axisX2[f]) && (this.chart.sessionVariables.axisX2[f] = {}), this.sessionVariables = this.chart.sessionVariables.axisX2[f]), this.options.interval || (this.intervalType =
            null), "theme2" === this.chart.theme && x(this.options.lineThickness) && (this.lineThickness = 2)) : "left" === d || "top" === d ?
            (this.optionsName = "axisY", x(this.chart.sessionVariables.axisY[f]) && (this.chart.sessionVariables.axisY[f] = {}), this.sessionVariables = this.chart.sessionVariables.axisY[f]) : (this.optionsName = "axisY2", x(this.chart.sessionVariables.axisY2[f]) && (this.chart.sessionVariables.axisY2[f] = {}), this.sessionVariables = this.chart.sessionVariables.axisY2[f]);
    ...
    var z = {},
        ...
        ta = 1,
        Q = Na ? Aa / ta : 1,
        Ba = window && window.location && window.location.href && window.location.href.indexOf && (-1 !== window.location.href.indexOf("canvasjs.com") || -1 !== window.location.href.indexOf("fenopix.com") || -1 !== window.location.href.indexOf("fiddle")),
        Ta = {
            reset: {
    ...
    (function() {
        ...
        z.fAWm = function(a) {
            if ("undefined" === typeof z.pro.sCH && !Ba) {
                ...
                c[z.fSDec(z.str.fTx)](z.fSDec(''), 2, a.height - 11 - 2)
            }
        }
    })();
    (function() {
                ...
                if (0 < s.length)
                    for (var u = 5 < p && q.bevelEnabled ? !0 : !1, f = 0; f < s.length; f++)
                        if (s[f].getTime ? n = s[f].x.getTime() : n = s[f].x, !(n < a.axisX.dataInfo.viewPortMin || n > a.axisX.dataInfo.viewPortMax) && null !== s[f].y && s[f].y.length && "number" === typeof s[f].y[0] && "number" === typeof s[f].y[1] && "number" === typeof s[f].y[2] && "number" === typeof s[f].y[3]) {
                            ...
                            if(q.risingColor.length) {
                                c.strokeStyle = s[f].y[0] <= s[f].y[3] ? q.risingColor : d;
                            } else {
                                c.strokeStyle = d;
                            }
                            c.beginPath();
                            ...
                            "candlestick" === q.type ?
                                (c.moveTo(g - x, k),
                                c.lineTo(g - x, Math.min(h, m)),
                                c.stroke(),
                                c.moveTo(g - x, Math.max(h, m)),
                                c.lineTo(g - x, l),
                                c.stroke(),
                                N(c, w, Math.min(h, m), y, Math.max(h, m),
                                s[f].y[0] <= s[f].y[3] ? q.risingColor : d,
                                0, d, u, u, !1, !1, q.fillOpacity),
                                v && (d = G(z),
                                b.stroke(),
                                b.moveTo(g - x, Math.max(h, m)),
                                b.lineTo(g - x, l),
                                b.stroke(),
                                N(b, w, Math.min(h, m), y, Math.max(h, m), d, 0, null, !1, !1, !1, !1)))

                                : "ohlc" === q.type && (c.moveTo(g - x, k), c.lineTo(g - x, l), c.stroke(), c.beginPath(), c.moveTo(g, h), c.lineTo(w, h), c.stroke(), c.beginPath(), c.moveTo(g, m), c.lineTo(y, m), c.stroke(), v && (d = G(z), b.strokeStyle = d, b.moveTo(g - x,
    ...
    B.prototype.createLabels =
        function() {
            ...
            if ("bottom" === this._position || "top" === this._position)
                l = this.logarithmic && !this.equidistantInterval && 2 <= this._labels.length ? this.lineCoordinates.width *
                Math.log(Math.min(this._labels[this._labels.length - 1].position / this._labels[this._labels.length - 2].position,
        ...
        for (b = 0; b < a.length; b++) {
            ...
            if ((f = this.chart._eventManager.objectMap[f.dataSeries.dataPointIds[f.index]]) && f.objectType && "dataPoint" === f.objectType) {
                var d = this.chart.data[f.dataSeriesIndex],
                    ...
                    h = f.dataPointIndex;
                !1 === g.highlightEnabled || !0 !== d.highlightEnabled && !0 !== g.highlightEnabled ||
                ("line1" === d.type || "stepLine" ===
                d.type || "spline" === d.type || "scatter" === d.type || "area" === d.type || "stepArea" === d.type || "splineArea" === d.type || "stackedArea" === d.type || "stackedArea100" === d.type || "rangeArea" === d.type || "rangeSplineArea" === d.type ?
                    (g = d.getMarkerProperties(h, f.x1, f.y1, this.chart.overlaidCanvasCtx),
                        ...
                        P.drawMarkers([g]),
                    "undefined" !== typeof f.y2 && (g = d.getMarkerProperties(h, f.x1, f.y2, this.chart.overlaidCanvasCtx),
                    g.size = Math.max(1.5 * g.size << 0, 10),
                        g.borderColor = g.borderColor || "#FFFFFF",
                        g.borderThickness = g.borderThickness || Math.ceil(0.1 * g.size),
                        P.drawMarkers([g]))) : "bubble" === d.type ? (g = d.getMarkerProperties(h, f.x1, f.y1, this.chart.overlaidCanvasCtx),
                        g.size = f.size, g.color = "white",
                        g.borderColor = "white",
                        c.globalAlpha = 0.3,
                        P.drawMarkers([g]),
                        c.globalAlpha = 1)
                        ...
                        : "column" === d.type || "stackedColumn" === d.type || "stackedColumn100" === d.type || "bar" === d.type || "rangeBar" === d.type || "stackedBar" === d.type || "stackedBar100" === d.type ||
                "rangeColumn" === d.type ? N(c, f.x1, f.y1, f.x2, f.y2, "white", 0, null, !1, !1, !1, !1, 0.3) : "pie" === d.type || "doughnut" === d.type ? Ha(c, f.center, f.radius, "white", d.type, f.startAngle, f.endAngle, 0.3, f.percentInnerRadius) :
                    "candlestick1" === d.type ?
                        (c.globalAlpha = 1,
                            c.strokeStyle = s[f].y[0] <= s[f].y[3] ? q.risingColor : f.color,


                            c.lineWidth = 0,
                            d = 0 === c.lineWidth % 2 ? 0 : 0.5,
                            c.beginPath(),
                            c.moveTo(f.x3 - d, Math.min(f.y2, f.y3)),
                            c.lineTo(f.x3 - d, Math.min(f.y1, f.y4)),
                            c.stroke(),
                            c.beginPath(),
                            c.moveTo(f.x3 - d, Math.max(f.y1, f.y4)),
                            c.lineTo(f.x3 - d,Math.max(f.y2, f.y3)),
                            c.stroke(),
                            N(c, f.x1, Math.min(f.y1, f.y4), f.x2, Math.max(f.y1, f.y4), "transparent", 0, f.color, !1, !1, !1, !1), c.globalAlpha = 1)
                        : "ohlc" === d.type && (c.globalAlpha = 1, c.strokeStyle = "red", c.lineWidth = 0, d = 0, c.beginPath(), c.moveTo(f.x3 - d, f.y2), c.lineTo(f.x3 - d, f.y3), 0, c.beginPath(), c.moveTo(f.x3, f.y1), c.lineTo(f.x1, f.y1), c.stroke(), c.beginPath(), c.moveTo(f.x3, f.y4), c.lineTo(f.x2, f.y4), c.stroke(), c.globalAlpha = 1))
            }
        }
        c.restore();
        c.globalAlpha =
    ...
    S.prototype.getToolTipInnerHTML = function(a) {
        ...
        if (h && (this.content && "function" === typeof this.content || this.contentFormatter)) a = {
            ...
            if ("line" === b.type || "stepLine" === b.type || "spline" === b.type || "area" === b.type || "stepArea" === b.type || "splineArea" === b.type || "column" ===
                ...
                d.toolTipContent ? d.toolTipContent : b.toolTipContent
                    ? b.toolTipContent : this.content && "function" !== typeof this.content ?
                        this.content : "<span style='\"" + (this.options.fontColor
